...
- **Cache-busting depends on which Dockerfile** — they're not all the same:
  - **Jekyll site image** — built from `~/Web/ClusteredPi/stacks/kevsrobots/dockerfile` (note: lowercase `dockerfile`, not `Dockerfile`). Standard command: `cd ~/ClusteredPi/stacks/kevsrobots && docker build -t 192.168.2.1:5000/kevsrobots:latest .` — Docker picks the lowercase file because that's all that's present (the `Dockerfile.*` variants are not used by `docker build .` since they have suffixes). Stage 1 does an `RUN git clone` inside the image, so **the layer must be cache-busted** or main updates won't appear in the image. The current dockerfile includes:
    ```dockerfile
    ADD https://github.com/kevinmcaleer/kevsrobots.com/commits/main.atom /tmp/main-sha.xml
    ARG CACHE_DATE=1   # manual override: --build-arg CACHE_DATE=$(date)
    RUN git clone --depth=1 https://...kevsrobots.com.git ...
    ```
    The `ADD` of a remote URL re-fetches every build; when main has new commits, the Atom feed's body changes and Docker invalidates this layer plus everything after it. **Don't use `https://api.github.com/repos/.../commits/main` for the cache-bust** — that endpoint is rate-limited to 60 requests/hr per IP for anonymous traffic, so after a busy day of builds the next `docker build` will fail with `failed to load cache key: invalid response status 403`. The `commits/main.atom` URL on `github.com` (not `api.github.com`) returns the same per-commit-changing content but is regular site traffic and not subject to that limit. **Symptom of missing cache-busting:** newly-added files (e.g. `web/assets/js/*.js`) 404 in production even though they're on `main` — Docker is reusing a cached `git clone` from before the file landed. **Symptom of API rate-limit:** build fails immediately at the `ADD` step with `403`.
...
- **ALTER-before-backfill ordering (lifespan)** - All column-adding `ALTER` helpers must run **before** any backfill/data helper that issues a *full-model ORM select* (e.g. `select(Part)`), because the ORM expands `select(Model)` to **every** mapped column — including ones you just added. If a backfill runs before its column's `ALTER`, Postgres raises `UndefinedColumnError` (`column parts.symbol_id does not exist`) **inside the FastAPI lifespan, so the app aborts startup → the container crash-loops → Cloudflare serves 502** (not 500). The SQLite test suite will NOT catch this: `create_all` builds every column up front, so order is irrelevant on SQLite but fatal on a legacy Postgres schema. **Rule: group all `add_*_if_missing` ALTERs first, then all `backfill_*` functions** (see the versioning block in `main.py`'s lifespan). Symptom: `curl https://projects.kevsrobots.com/api/projects/my/list` returns a Cloudflare **502** even for a bogus token (a healthy API returns 401) — the origin is down because startup aborted. Since the helpers are idempotent, the partial migration state from the crashed boot is harmless: just fix the order and redeploy.
